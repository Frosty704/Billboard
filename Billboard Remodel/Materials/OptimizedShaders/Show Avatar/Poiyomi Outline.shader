Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/4f9cbfcf113f330419f0476db79c4bb4"
{
	Properties
	{
...
#define PROP_DECALEMISSIONSTRENGTH 12.62
...
#define PROP_DECALBLENDALPHA 0.532
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(3) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.001953125,0.00390625,512,256).zw)) * float(0.25);
						poiFragData.alpha = (poiFragData.alpha - float(1.001)) / max(fwidth(poiFragData.alpha), 0.0001) + float(1.001);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - float(1.001));
