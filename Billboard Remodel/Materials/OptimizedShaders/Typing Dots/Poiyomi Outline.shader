Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/ff60bee8ab1f8074280af0b913daf7d8"
{
	Properties
	{
...
#define PROP_FLIPBOOKCOLORREPLACES 1
...
#define PROP_FLIPBOOKHUESHIFTSPEED 5
#define PROP_FLIPBOOKHUESHIFT 1
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					if (float(30) != 0)
					{
						currentFrame = (_Time.y / (1 / float(30))) % float(30);
					}
				}
				else
				{
					currentFrame = fmod(float(0), float(30));
				}
				...
				if (float(0))
				{
					float4 flipbookNextPixel = UNITY_SAMPLE_TEX2DARRAY(_FlipbookTexArray, float3(TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0), floor((currentFrame + 1) % float(30))));
				...
				if (float(0))
				{
					flipBookPixel.rgb = hueShift(flipBookPixel.rgb, float(1) + _Time.x * float(5));
				}
