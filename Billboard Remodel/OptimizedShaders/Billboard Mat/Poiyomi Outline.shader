Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/8816bf3639f5661449d84b2760d11d37"
{
	Properties
	{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					clip(OutlineMask * float(0.75) - 0.001);
				}
				...
				if (float(1))
				{
					poiFragData.baseColor = hueShift(poiFragData.baseColor, float(1) +float(4) * _Time.x);
				}
				poiFragData.emission += poiFragData.baseColor * float(1);
			}
			#endif
			#ifdef VIGNETTE_MASKED
			#ifdef _LIGHTINGMODE_CLOTH
