Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/78746b1c9af232f4f8f276b0a1f4738b"
{
	Properties
	{
...
#define PROP_DISSOLVETOEMISSIONSTRENGTH 1
...
#define PROP_DISSOLVEDETAILSTRENGTH 0.4
#define PROP_DISSOLVEALPHA 0.5
...
#define PROP_FLIPBOOKTOTALFRAMES 30
#define PROP_FLIPBOOKFPS 30
#define PROP_FLIPBOOKTILED 0
#define PROP_FLIPBOOKEMISSIONSTRENGTH 1
...
#define PROP_EMISSIONHUESHIFTSPEED 2
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float3 offset = outlineNormal * (float(0.2) * float(0) / 100) * outlineMask * distanceOffset;
				if (float(0) == 2)
				{
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(0) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(0.0004882813,0.0009765625,2048,1024).zw)) * float(0.25);
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				poiFragData.emission += lerp(0, dissolveToTexture * float(1), dissolveAlpha) + lerp(0, edgeColor.rgb * float(0), smoothstep(0, 1 - float(0.5) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.8705506,0.8705506,0.8705506,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(1)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.8705506,0.8705506,0.8705506,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.8705506,0.8705506,0.8705506,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				emissionColor0 = hueShift(emissionColor0, frac(float(1) + float(2) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(1))
				{
					poiLight.directColor = min(poiLight.directColor, float(0.5));
					poiLight.indirectColor = min(poiLight.indirectColor, float(0.5));
				}
				if (float(0))
				{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0)), float4(1,0,0,1).a);
				if (float(0) == 0)
				{
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(float4(poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0)), float4(1,0,0,1).a), poiFragData.baseColor, float(0)) * col;
				}
				if (float(0) == 2)
				{
