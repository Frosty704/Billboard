Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/d809fda70863faa448315442bf0a95ae"
{
	Properties
	{
...
#define PROP_OUTLINEHUEOFFSETSPEED 5
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float3 offset = outlineNormal * (float(0.002) * _EnableOutlines / 100) * outlineMask * distanceOffset;
				if (float(0) == 2)
				{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					clip(OutlineMask * float(0.002) - 0.001);
				}
				...
				if (float(1))
				{
					poiFragData.baseColor = hueShift(poiFragData.baseColor, float(1) +float(5) * _Time.x);
				}
