Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/d1fee5db8603be94c931c70b2bdf40b9"
{
	Properties
	{
...
#define PROP_OUTLINEHUEOFFSETSPEED 1.5
...
#define PROP_DISSOLVEEDGEWIDTH 0.28
#define PROP_DISSOLVEEDGEHARDNESS 0.426
...
#define PROP_DISSOLVETOEMISSIONSTRENGTH 5
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			v2f vert(appdata v)
			{
				...
				float3 offset = outlineNormal * (float(0.001) * _EnableOutlines / 100) * outlineMask * distanceOffset;
				if (float(0) == 2)
				{
...
#define PROPM_END_OUTLINEBLENDING 0
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					clip(OutlineMask * float(0.001) - 0.001);
				}
				...
				if (float(1))
				{
					poiFragData.baseColor = hueShift(poiFragData.baseColor, float(1) +float(1.5) * _Time.x);
				}
